...
__global__ void cuAddQNormAndSqrt(float *dist, int width, int pitch, float *q, int k){
  ...
  if (xIndex<width && yIndex<k)
    ...
    dist[yIndex*pitch + xIndex] = dist[yIndex*pitch + xIndex] + q[xIndex];
}
...
void knn(float* ref_host, int ref_width, float* query_host, int query_width, int height, int k, float* dist_host, int* ind_host){
  ...
  float compute_distance_time = (kernel1_time + kernel2_time + kernel3_time +
                                kernel4_time + kernel6_time) / NUM_ITERATION;
  float select_neighbor_time = kernel5_time / NUM_ITERATION;
...
int main(int argc, char **argv) {
  if (argc == 1) {
    printf("./knn_cublas_with_indexes (dim) (query_nb) (ref_nb) (k) "
           "(query_file) (reference_file)\n");
    return 0;
  }
  assert(argc == 7);
  ...
  int    dim        = atoi(argv[1]);  // Dimension of points,    max=8192
